...
class IPCMqttProxyTest {
    ...
    @Test
    void GIVEN_receive_only_WHEN_subscribe_THEN_local_only_registered_and_message_received() throws Exception {
        ...
        SubscribeToIoTCoreRequest subscribeToIoTCoreRequest = new SubscribeToIoTCoreRequest();
        subscribeToIoTCoreRequest.setTopicName(subscribeTopic);
        // RECEIVE_ONLY (direct-message) mode with NO qos -- must not be rejected for missing qos.
        ...
        Subscribe capturedSubscribeRequest = subscribeRequestArgumentCaptor.getValue();
        assertThat(capturedSubscribeRequest.getTopic(), is(subscribeTopic));
        // Local-only registration: no cloud SUBSCRIBE is issued for this request.
        assertThat(capturedSubscribeRequest.isSkipCloudSubscribe(), is(true));

        // A message delivered to the captured callback still streams back to the component over IPC.
        Consumer<Publish> callback = capturedSubscribeRequest.getCallback();
    ...
    @Test
    void GIVEN_receive_only_with_qos_WHEN_subscribe_THEN_qos_ignored_and_local_only() throws Exception {
        lenient().when(mqttClient.unsubscribe(any(Unsubscribe.class)))
                .thenReturn(CompletableFuture.completedFuture(null));
        ...
        SubscribeToIoTCoreRequest subscribeToIoTCoreRequest = new SubscribeToIoTCoreRequest();
        subscribeToIoTCoreRequest.setTopicName(subscribeTopic);
        // A supplied qos must be silently ignored in RECEIVE_ONLY mode (no validation, no error).
        subscribeToIoTCoreRequest.setQos(QOS.AT_LEAST_ONCE);
        subscribeToIoTCoreRequest.setSubscriptionMode(SubscriptionMode.RECEIVE_ONLY);
    ...
    void GIVEN_receive_only_on_unauthorized_topic_WHEN_subscribe_THEN_client_gets_unauthorized(
        ...
        // Subscribe is a streaming operation, so the client requires a stream handler even when we expect the
        // request itself to be rejected (auth fails before any stream event is produced).
        StreamResponseHandler<IoTCoreMessage> streamResponseHandler = new StreamResponseHandler<IoTCoreMessage>() {
            @Override
            public void onStreamEvent(IoTCoreMessage streamEvent) {
            }
        ...
        // Authorization is checked before the mode branch, so no subscription is ever attempted.
