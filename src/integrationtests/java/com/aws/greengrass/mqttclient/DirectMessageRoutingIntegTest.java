...
/**
 * End-to-end test for a RECEIVE_ONLY (local-only) subscribe: it runs over real IPC through the real
 * {@link MqttProxyIPCAgent} handler AND the real {@link MqttClient} router and local-only store. Unlike
 * {@code IPCMqttProxyTest} (which mocks {@code MqttClient}), this wires a real {@code MqttClient} with a mocked
 * CRT MQTT5 stack into the kernel, so the local-only registration and the router's local-only delivery pass
...
public class DirectMessageRoutingIntegTest extends BaseITCase {
    ...
    void before() throws Exception {
        ...
        // Mocked CRT MQTT5 stack: the real MqttClient never opens a cloud connection, and we assert below that
        // no cloud SUBSCRIBE is ever issued for a RECEIVE_ONLY registration.
    ...
    void GIVEN_receive_only_subscribe_over_ipc_WHEN_inbound_message_THEN_routed_to_component_with_no_cloud_subscribe()
        ...
        // Real IPC round-trip: handler takes the RECEIVE_ONLY branch and the real MqttClient records a
        // local-only registration (no cloud SUBSCRIBE). A successful response proves the registration stuck.
        ...
        // Inject an inbound message through the REAL router. For a local-only sub with no covering cloud
        // subscription, delivery is not gated on the receiving client's identity, so any client handle works.
        ...
        // The message traversed: real router local-only pass -> real IPC handler callback -> streamed to component.
        ...
        // End-to-end proof of "local only": the real MqttClient never issued a cloud SUBSCRIBE.
